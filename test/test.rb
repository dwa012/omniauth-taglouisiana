...
class ClientTest < StrategyTestCase
  test 'has correct Facebook site' do
    assert_equal 'https://graph.taglouisiana.com', strategy.client.site
  end

  test 'has correct authorize url' do
    assert_equal 'https://www.taglouisiana.com/dialog/oauth', strategy.client.options[:authorize_url]
  end
...
class CallbackUrlTest < StrategyTestCase
  test "returns the default callback url" do
    ...
    assert_equal "#{url_base}/auth/taglouisiana/callback", strategy.callback_url
  end
...
class InfoTest < StrategyTestCase
  test 'returns the secure taglouisiana avatar url when `secure_image_url` option is specified' do
    @options = { :secure_image_url => true }
    raw_info = { 'name' => 'Fred Smith', 'id' => '321' }
    strategy.stubs(:raw_info).returns(raw_info)
    assert_equal 'https://graph.taglouisiana.com/321/picture', strategy.info['image']
  end
  ...
  test 'returns the image with size specified in the `image_size` option' do
    ...
    raw_info = { 'name' => 'Fred Smith', 'id' => '321' }
    strategy.stubs(:raw_info).returns(raw_info)
    assert_equal 'http://graph.taglouisiana.com/321/picture?type=normal', strategy.info['image']
  end
  ...
  test 'returns the image with width and height specified in the `image_size` option' do
    ...
    assert_equal 'http://graph.taglouisiana.com/321/picture', path
...
class InfoTestOptionalDataPresent < StrategyTestCase
  ...
  test 'returns the taglouisiana avatar url' do
    @raw_info['id'] = '321'
    assert_equal 'http://graph.taglouisiana.com/321/picture', strategy.info['image']
  end

  test 'returns the Facebook link as the Facebook url' do
    @raw_info['link'] = 'http://www.taglouisiana.com/fredsmith'
    assert_kind_of Hash, strategy.info['urls']
    assert_equal 'http://www.taglouisiana.com/fredsmith', strategy.info['urls']['Facebook']
  end
...
class RawInfoTest < StrategyTestCase
  ...
  test 'performs a GET to https://graph.taglouisiana.com/me' do
